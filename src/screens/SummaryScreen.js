      ...
      <View style={s.content}>
        ...
        <View style={s.statsRow}>
          <View style={[s.statCard, { borderColor: theme.border }]}>
            ...
            <Text style={[s.statValue, { color: theme.primary }]}>{expenses.length}</Text>
          </View>
          <View style={[s.statCard, { borderColor: theme.border }]}>
        ...
        {topExpense && (
          <>
            ...
            <View style={[s.topCard, { borderColor: theme.border, backgroundColor: theme.card }]}>
              <View style={s.topCardLeft}>
                <Text style={[s.topCatBadge, {
                  backgroundColor: theme.categories[topExpense.category]?.bg,
                  color: theme.categories[topExpense.category]?.color,
                }]}>
                  {topExpense.category}
                </Text>
                <Text style={s.topTitle} numberOfLines={1}>{topExpense.title}</Text>
                <Text style={s.topDate}>{topExpense.date}</Text>
              </View>
              <Text style={[s.topAmount, { color: theme.categories[topExpense.category]?.color || theme.primary }]}>
                Rs {parseFloat(topExpense.amount).toLocaleString()}
              </Text>
            </View>
          </>
        )}
        ...
        {categoryData.length === 0 ? (
          ...
          categoryData.map((cat, index) => {
            ...
            const catStyle = theme.categories[cat.name] || { color: theme.primary, bg: theme.primaryGlow };
            return (
              <View key={index} style={[s.catCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                <View style={s.catTop}>
                  <View style={s.catLeft}>
                    <View style={[s.catDot, { backgroundColor: catStyle.color }]} />
                    <Text style={s.catName}>{cat.name}</Text>
                  </View>
                  <View style={s.catRight}>
                    <Text style={[s.catAmount, { color: catStyle.color }]}>
                      Rs {cat.amount.toLocaleString('en', { maximumFractionDigits: 0 })}
                    </Text>
